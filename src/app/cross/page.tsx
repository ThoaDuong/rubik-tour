...
import { crossCases, CROSS_GROUPS } from '@/data/cross';
...
export default function CrossPage() {
  ...
  const filtered = crossCases.filter((c) => {
  ...
  const learningCount = isMounted
    ? crossCases.filter((c) => getStatus(c.id) === 'learning').length
    : 0;
  const learnedCount = isMounted
    ? crossCases.filter((c) => getStatus(c.id) === 'learned').length
    : 0;
  ...
  return (
    <div className="animate-fade-in pb-16">
      ...
      <div className="p-6 md:p-10 pb-0">
        <div className="text-emerald-600 text-[11px] font-bold tracking-widest uppercase mb-2">
          Thư viện
        </div>
        <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-text-primary">
          Cross — White Cross / Bottom Layer
        </h2>
        <p className="text-text-secondary text-sm mt-1.5 flex items-center gap-2 flex-wrap">
          <span>Giải 4 cạnh trắng về mặt dưới. {crossCases.length} kỹ thuật</span>
          <span>•</span>
      ...
      <div className="px-6 md:px-10 pt-5 flex items-center gap-2 flex-wrap">
        {CROSS_GROUPS.map((g) => {
          ...
          if (g.id === 'learning') {
            ...
            count = crossCases.filter((c) => c.group === g.id).length;
          }

          let activeStyle = 'bg-emerald-500/10 border-emerald-500 text-emerald-700 shadow-xs';
          if (g.id === 'learning') {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold cursor-pointer border transition-all duration-200 flex items-center gap-1.5 ${
                isActive
                  ? activeStyle
                  : 'bg-white border-border-subtle text-text-secondary hover:border-emerald-500 hover:text-text-primary'
              }`}
            >
              {g.id === 'learning' && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-6 md:p-10 pt-6">
        {filtered.map((c) => (
          <AlgCard key={c.id} case_={c} type="cross" />
        ))}
        {filtered.length === 0 && (
